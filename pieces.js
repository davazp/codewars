// See pieces.txt for some test cases

// Fill a connected component of OLD-valued characters with NEWVALUE.
...
function normalize (piece){
  while (piece.every(function(line){ return line[0]==' '; })){
    piece.forEach(function(line,i){
      piece[i] = piece[i].slice(1);
    });
  }
  return piece.filter(function(line){
    return !line.every(function(place){ return place==' '; });
  });
}
...
function extractX (lines, i, j) {
  ...
  // Remove unnecesary + in the component.
  piece.forEach(function(line,i){
...
function breakPieces (shape){
  // Bidimensional array representation
  var lines = [];
  shape.split("\n").forEach(function(line,i){
    lines[i] = line.split('');
  });
  // Fill the component with Xs. Then extract the set of characters
  // with X in an adjacent position.
  var pieces = [];
